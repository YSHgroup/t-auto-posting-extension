...
class DemoStore:
    ...
    def dashboard(self) -> Dashboard:
        return Dashboard(groups=len(self.data.groups), active_feed=sum(item.enabled for item in self.feed), total_posts=len(self.posts), posts_today=sum(item.status == "success" and item.posted_at.date() == datetime.now(UTC).date() for item in self.history), skipped_posts=sum(item.status == "skipped" for item in self.history), replies=3, unread_notifications=3, investors=12, partners=19)
